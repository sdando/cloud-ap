...
public final class HadoopUtil {
  ...
  public static Job prepareJob(Path inputPath,
    ...
    ((JobConf)jobConf).setNumMapTasks(8);
    if (mapperKey != null) {
      job.setMapOutputKeyClass(mapperKey);
    }
    if (mapperValue != null) {
  ...
  public static String readPrefer(Path path,Configuration conf) throws IOException{
	    FileSystem fs = path.getFileSystem(conf);
	    SequenceFile.Reader reader = null;
	    FloatWritable prefer=null;
	    ...
	    try {
	      ...
	      prefer=(FloatWritable)value;
	    } finally {
	      IOUtils.closeStream(reader);
	    }   
	    return String.valueOf(prefer.get());
  }
}
